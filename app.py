...
additional_features = {
    ...
    "Age (in months)": [str(i) for i in range(1, 229)],
    "Jaundice": ["Yes", "No"],
...
if st.session_state.question_index < len(questions):
    ...
    st.write(f'**Q{st.session_state.question_index + 1}: {questions[st.session_state.question_index]}**')
    
    response = st.radio("Select an option:", list(response_mapping.keys()), index=None)
...
# Additional Features Input
    ...
    for feature, options in additional_features.items():
        st.session_state.additional_responses[feature] = st.selectbox(feature, options, index=None)
